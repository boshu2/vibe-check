...
/**
 * Calculate average debug spiral duration.
 *
 * Note on zero-spiral case: Unlike other metrics that return 'medium' for
 * zero/empty data (insufficient data to measure), this returns 'elite' because
 * "no spirals detected" is a meaningful positive result — we measured and found
 * no negative patterns. This is intentionally different from "no commits to analyze".
 */
export function calculateDebugSpiralDuration(chains: FixChain[]): MetricResult {
  const spirals = chains.filter((c) => c.isSpiral);

  // No spirals = positive result (not "insufficient data")
  if (spirals.length === 0) {
    return {
      value: 0,
      unit: 'min',

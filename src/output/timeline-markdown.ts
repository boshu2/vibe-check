...
export function formatTimelineMarkdown(timeline: TimelineResult): string {
  ...
  if (timeline.postDeleteSprint?.detected) {
    lines.push(`- ⚡ **Post-delete sprint:** ${timeline.postDeleteSprint.message}`);
  }
  if (timeline.flowStates > 0) {
    ...
    lines.push(`- 🌊 **Flow states:** ${timeline.flowStates} detected (avg ${Math.round(avgFlowDuration)} min)`);
  }
  if (timeline.detours?.detected) {
    lines.push(`- 🚧 **Detours:** ${timeline.detours.message}`);
  }
  if (timeline.lateNightSpirals?.detected) {
    lines.push(`- 🌙 **Late-night spirals:** ${timeline.lateNightSpirals.message}`);
  }
  if (timeline.thrashing?.detected) {
    lines.push(`- 🔄 **Thrashing:** ${timeline.thrashing.message}`);
  }
  if (timeline.totalSpirals > 0) {
    lines.push(`- ⚠️ **Debug spirals:** ${timeline.totalSpirals} detected`);
  }

  // Productivity ratio
  const wallClockHours = timeline.totalDays * 24;
  const efficiencyPercent = Math.round((activeHours / wallClockHours) * 100);
  lines.push(`- ⏱️ **Active time:** ${activeHours}h of ${wallClockHours}h (${efficiencyPercent}% efficiency)`);
  ...
  if (bestDay.dayScore && bestDay.dayScore >= 80) {
    lines.push(`- 🏆 **Best day:** ${bestDay.displayDate} (${bestDay.dayScore}%)`);
  }
...
function formatDayMarkdown(day: TimelineDay): string {
  ...
  lines.push(`### ${day.displayDate} — ${day.dayScore}% ${day.dayRating}${trophy}`);
  lines.push('');
  ...
  for (const session of day.sessions) {
    ...
    const spiralIcon = session.spirals.length > 0 ? ' ⚠️' : '';

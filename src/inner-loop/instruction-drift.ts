 ...
 *
 * Uses the first N commits (configurable via workingSetCommitCount) to
 * establish the "working set" - files that are in scope for this session.
 * Refactoring commits that touch files outside this set are flagged.
 *
 * Note: If your first commits are setup/boilerplate, increase workingSetCommitCount
 * or define explicit sessionScope to avoid false positives.
 */
function detectUnrequestedRefactors(
  ...
  // Build "normal" working set from first N commits (configurable)
  const workingSet = new Set<string>();
  const firstCommitCount = Math.min(config.workingSetCommitCount, events.length);
 ...
 *
 * The commitCount parameter (or config.workingSetCommitCount) determines
 * how many initial commits define the working set. Increase this if your
 * first commits are setup/boilerplate that don't represent the actual scope.
 *
 * @param events - Timeline events to analyze
 * @param filesPerCommit - Map of commit hash to files changed
 * @param commitCount - Number of commits to use for inference (default: 3)
 */

...
describe('detectTestsPassingLie', () => {
  ...
  it('does NOT flag bare "fix:" commits as success claims', () => {
    // This is the key edge case: iterative development with conventional commits
    // "fix(auth): add null check" is NOT a success claim - just describes work
    const events = [
      makeEvent('abc123', new Date('2024-01-01T10:00:00'), 'fix', 'auth', 'fix(auth): add null check'),
      makeEvent('def456', new Date('2024-01-01T10:15:00'), 'fix', 'auth', 'fix(auth): handle edge case'),
      makeEvent('ghi789', new Date('2024-01-01T10:30:00'), 'fix', 'auth', 'fix(auth): improve validation'),
    ];
    ...
    // None of these are success claims - just iterative fixes
    expect(result.detected).toBe(false);
    expect(result.totalLies).toBe(0);
  });

  it('flags explicit success claims like "working", "done", "it works"', () => {
    const events = [
      makeEvent('abc123', new Date('2024-01-01T10:00:00'), 'fix', 'auth', 'fix: auth done'),
      makeEvent('def456', new Date('2024-01-01T10:10:00'), 'fix', 'auth', 'fix: forgot edge case'),
    ];
  ...
  it('flags "tests pass" but not "passing data to function"', () => {
    // "tests pass" = success claim, "passing data" = not a success claim
    const events = [
      makeEvent('abc123', new Date('2024-01-01T10:00:00'), 'fix', 'api', 'fix: passing data to handler'),
      makeEvent('def456', new Date('2024-01-01T10:15:00'), 'fix', 'api', 'fix: adjust handler'),
    ];

    const filesPerCommit = new Map([
      ['abc123', ['src/api/handler.ts']],
      ['def456', ['src/api/handler.ts']],
    ]);

    const result = detectTestsPassingLie(events, filesPerCommit);

    // "passing data" is not a success claim
    expect(result.detected).toBe(false);
  });

  it('flags "fixed it" and "fixed the bug" as success claims', () => {
    const events = [
      makeEvent('abc123', new Date('2024-01-01T10:00:00'), 'fix', 'auth', 'fixed the bug'),
      makeEvent('def456', new Date('2024-01-01T10:10:00'), 'fix', 'auth', 'fix: oops missed something'),
    ];
